# Hall basis for F2 = <a,b>, weight <= 6
...
hallBasis := [
    ...
    rec( index := 15, weight := 6, label := "[[[b,a],b],[[b,a],a]]", elt := Comm(Comm(Comm(F.2,F.1),F.2),Comm(Comm(F.2,F.1),F.1)) ),
    rec( index := 16, weight := 6, label := "[[[[b,a],a],a],[b,a]]", elt := Comm(Comm(Comm(Comm(F.2,F.1),F.1),F.1),Comm(F.2,F.1)) ),
    rec( index := 17, weight := 6, label := "[[[[b,a],a],b],[b,a]]", elt := Comm(Comm(Comm(Comm(F.2,F.1),F.1),F.2),Comm(F.2,F.1)) ),
    rec( index := 18, weight := 6, label := "[[[[b,a],b],b],[b,a]]", elt := Comm(Comm(Comm(Comm(F.2,F.1),F.2),F.2),Comm(F.2,F.1)) ),
    rec( index := 19, weight := 6, label := "[[[[[b,a],a],a],a],a]", elt := Comm(Comm(Comm(Comm(Comm(F.2,F.1),F.1),F.1),F.1),F.1) ),
    rec( index := 20, weight := 6, label := "[[[[[b,a],a],a],a],b]", elt := Comm(Comm(Comm(Comm(Comm(F.2,F.1),F.1),F.1),F.1),F.2) ),
    rec( index := 21, weight := 6, label := "[[[[[b,a],a],a],b],b]", elt := Comm(Comm(Comm(Comm(Comm(F.2,F.1),F.1),F.1),F.2),F.2) ),
    rec( index := 22, weight := 6, label := "[[[[[b,a],a],b],b],b]", elt := Comm(Comm(Comm(Comm(Comm(F.2,F.1),F.1),F.2),F.2),F.2) ),
    rec( index := 23, weight := 6, label := "[[[[[b,a],b],b],b],b]", elt := Comm(Comm(Comm(Comm(Comm(F.2,F.1),F.2),F.2),F.2),F.2) )
];
