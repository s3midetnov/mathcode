...
gens_3_small = [FW6([1]), FW6([2]), FW6([3]), FW6([1, 2, -1, -2]), FW6([1, 3, -1, -3]), FW6([2, 3, -2, -3])]
...
true && begin
    println("lifting twoRelators.txt → twoRelatorsF3.txt")
    im_ball = collect(ball(gens_3_small, 3))
    seen = Set{String}()
    cntr = Ref(0)
    open("twoRelatorsF3.txt", "w") do file
        for raw in shuffle!(readlines("twoRelators.txt"))
            parts = split(raw, ", ")
            ...
            for im1 in im_ball, im2 in im_ball
                ...
                line = "$(images[1]), $(images[2]), $(images[3]), $(images[4])"
